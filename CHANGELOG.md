...
- Border mask for pen plotter safety: `src/border-mask.ts` exports `applyBorderMask(svg, canvasConfig, { inset, drawBorder })`, which clips artwork to a rectangle inset from the canvas edge so the pen never runs off the paper. Because plotter drivers ignore SVG `clip-path` and plot raw geometry, the mask rewrites coordinates directly (Liang–Barsky clipping of `<line>`, `<polyline>`, `<polygon>`, and `<rect>` strokes, splitting chains that exit and re-enter into multiple polylines); shapes it can't rewrite (`<path>`, circles, transformed or filled shapes) get a visual clip-path plus a console warning. `drawBorder: true` also plots the border rectangle as a frame. Calibration marks are exempt. `noise-field-study` demos it with a Border Mask mode dropdown (Off / Mask only / Mask + border) and a Border Inset slider. The controls are now standard equipment: artworks generated by the wizard or `npm run new` include the dropdown, the slider, and the `applyBorderMask` call by default (right after the calibration toggle), and every existing artwork in `art/` has been retrofitted with the same controls and call (defaults: Mask + border, 24 px inset)
