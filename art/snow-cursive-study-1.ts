 ...
 *
 * Part one of a multi-part study: a single snowflake centered on the
 * canvas. One arm is grown from the center — a main stem that sprouts
 * mirrored branch pairs, which sprout their own pairs, and so on — and
 * that one arm is then replicated around the center point. In the
 * lettering mode every line of the skeleton is replaced by words from
 * the WORDS list, set in Secondhand Cursive, one or more copies per
 * segment depending on the word size.
 */
...
export const meta = {
  title: 'Snow Cursive Study 1',
  description: 'A single snowflake whose branches are written in cursive',
};
...
export function draw(values: Values, canvasConfig: CanvasConfig): SVGElement {
  ...
  if (showCalibration) {
    ...
    if (renderMode !== 'lines') {
      ...
      if (total > MAX_WORDS) {
        console.warn(
          `Snow Cursive Study 1: ${total} words requested, capping at ${MAX_WORDS}. ` +
